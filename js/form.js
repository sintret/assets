...
function removeimage(el) {
    let $this = $(el);
    $this.parents().parents().find('input[type="file"]').attr("data-test","test").attr("value","").attr("type","text")
...
function loadFile(input, elem="") {
    ...
    if (input.files && input.files[0] && (ext == "gif" || ext == "png" || ext == "jpeg" || ext == "jpg" || ext == "bmp" || ext == "svg" || ext == "webp")) {
        ...
        reader.onload = function (e) {
            if(parentTag == "TD") {
                $this.closest("td").find("img").attr("src",e.target.result).attr("width",`${width}px`).addClass("boxy");
            } else {
                $this.closest("div").find("img").attr("src",e.target.result).attr("width",`${width}px`).addClass("boxy");
            }
        };
        ...
        if(parentTag == "TD") {
            $this.closest("td").find("img").attr("src",`/img/${file}`).attr("height","45px").attr("width","45px").addClass("boxy-small");
        } else {
            $this.closest("div").find("img").attr("src",`/img/${file}`).attr("height","45px").attr("width","45px").addClass("boxy-small");
        }
    }
}
